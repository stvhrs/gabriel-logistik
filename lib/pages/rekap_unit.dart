...
class _RekapUnitState extends State<RekapUnit> {
  ...
  void didChangeDependencies() {
    for (var element in Provider.of<ProviderData>(context,listen: false).listTransaksi) {
  ...
  test() {
    for (var moon in list) {
      ...
      transaksiBulanIni = Provider.of<ProviderData>(context,listen: false)
          ...
          .toList();
      List<Perbaikan> bulanPerbaikan = Provider.of<ProviderData>(context,listen: false)
          .backupListPerbaikan
          .where((element) =>
     ...
     listKas.add(RekapModel(totalSisa, totalKeluar, totalOngkos, totalPerbaikan, totalSisa-totalPerbaikan, moon));
